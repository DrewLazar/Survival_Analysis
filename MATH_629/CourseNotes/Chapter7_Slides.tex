...
The bears data set was collected in the Northwest Maine. Blood sample were taken from the bears. The covariate Iron in the data set is the amount micrograms of iron in the bears blood per deciliter. Iron.group groups the Iron data set into three groups, low (Iron.group=1), medium (Iron.group=2) and high (Iron.group=3). The response is the number of year of survival after 10 years of life.
\begin{enumerate}
\item Plot KM curves of Iron.group.
\item Test the appropriateness of the log-logistic model with fixed $p$ for just Iron.group in the model.
\item Fit an AFT log-logistic model with just Iron and give the AF. Give the $95\%$ CI for the AF. Is Iron a significant predictor?
