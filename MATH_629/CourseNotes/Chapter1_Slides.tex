...
\begin{frame}
\begin{block}{Example 7.2 - Exponential Distribution as PH Model} 
\begin{enumerate}[ ]
\item Remission data ($n=42$). One covariate treatment status (TR). 
\item 21 patients given treatment (TR=1). 21 patients given placebo (TR=0). 
\end{enumerate} 
Assuming an exponential distribution we have
\[ S(t;TR)= \exp(\beta_0 + \beta_1 TRT)
\]
Our hazard ratio is 
\[
