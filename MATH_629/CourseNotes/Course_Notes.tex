...
\end{align*}
For $t_{f}>0$ we find estimate  $\hat{S}(t_{(f)})$, accordingly and recursively, as  
\[
 \hat{S}(t_{(f)}) = \widehat{P(T>t_{(f)} | T \ge t_{(f)})} \hat{S}(t_{(f-1)})
 \]
This is known as the  \textbf{Kaplan-Meirer} or \textbf{ product limit estimators } of the survival curve.  
