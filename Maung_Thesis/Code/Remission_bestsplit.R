...
objvals <- rep(0, nrow(lines))
for (i in 1:nrow(lines)) {
  
  vi = c(-lines[i, 2], lines[i, 1], 1)
  
  phipmi <- phipmcount(Remission.Zmatrix, vi, Remission.poms)
  
  objvals[i] = kretowska.dipolarcriterion(Remission.Zmatrix, vi, phipmi)
  
}

which.min(objvals)

which(objvals == min(objvals))

plot(logWBC ~ TR, data = Remission, pch = 20)
abline(a = lines[273, 2], b = -lines[273, 1])
